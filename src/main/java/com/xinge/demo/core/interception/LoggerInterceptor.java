...
public class LoggerInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (requestUri != null && handler instanceof HandlerMethod && log.isInfoEnabled()) {
            ...
            log.info("--> HttpRequest [{}:{}:{}]:[{}]:{}", request.getRemoteHost(), request.getRemotePort(), request.getSession().getId(), request.getRequestURI(), debugMap);
        }
        return true;
    }
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        if (requestUri != null && handler instanceof HandlerMethod && log.isInfoEnabled()) {
            ...
            String debugParam = "";
            if (resultMap != null) {
                debugParam = JSON.toJSONString(resultMap);
                if (debugParam.length() > REQUEST_PARAM_SIZE) {
                    debugParam = debugParam.substring(0, REQUEST_PARAM_SIZE) + "......";
                }
            }
            log.info("<-- HttpResponse [{}:{}:{}]:[{}]:{}", request.getRemoteHost(), request.getRemotePort(), request.getSession().getId(), request.getRequestURI(), debugParam);
        }
    }
